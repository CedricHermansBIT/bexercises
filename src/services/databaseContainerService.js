...
async function startMariaDBContainer(tmpdir, fixtures = [], dockerImage = runtimeImages.mariadb) {
    ...
    const dockerArgs = [
        ...
        '--network', 'none',
		'--read-only',
		'--tmpfs', `/var/lib/mysql:rw,nosuid,size=${config.docker.databaseStorageSize},mode=1777`,
		'--tmpfs', '/run/mysqld:rw,nosuid,size=8m,mode=1777',
		'--tmpfs', '/tmp:rw,nosuid,size=64m,mode=1777',
        '--memory', config.docker.databaseMemory,
        '--cpus', config.docker.cpus,
        '--pids-limit', String(config.docker.databasePidsLimit),
        '--cap-drop', 'ALL',
...
async function startMongoDBContainer(tmpdir, fixtures = [], dockerImage = runtimeImages.mongodb) {
    ...
    const dockerArgs = [
        ...
        '--network', 'none',
		'--read-only',
		'--tmpfs', `/data/db:rw,nosuid,size=${config.docker.databaseStorageSize},mode=1777`,
		'--tmpfs', '/data/configdb:rw,nosuid,size=32m,mode=1777',
		'--tmpfs', '/tmp:rw,nosuid,size=64m,mode=1777',
        '--memory', config.docker.databaseMemory,
        '--cpus', config.docker.cpus,
        '--pids-limit', String(config.docker.databasePidsLimit),

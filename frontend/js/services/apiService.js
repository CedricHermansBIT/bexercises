...
class ApiService {
    ...
    async runTestCase(solution, testCase) {
        console.log('[API] runTestCase called with:', {
            languageId: testCase.languageId,
            exerciseType: testCase.exerciseType,
            hasValidationQuery: !!testCase.validationQuery,
            validationQueryPreview: testCase.validationQuery?.substring(0, 50)
        });
        ...
        console.log('[API] runTestCase result:', {
            hasOutput: !!result.output,
            outputLength: result.output?.length || 0,
            hasValidationOutput: !!result.validationOutput,
            validationOutputLength: result.validationOutput?.length || 0,
